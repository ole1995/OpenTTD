...
static void AircraftEventHandler_HeliEndLanding(Aircraft *v, const AirportFTAClass *apc)
{
	 ...
	 * 1. in case all terminals/helipads are busy (AirportFindFreeTerminal() returns false) or
	 * 2. not going for terminal (but depot, no order),
...
static bool AirportHasBlock(Aircraft *v, const AirportFTA *current_pos, const AirportFTAClass *apc)
{
	...
	if (apc->layout[current_pos->position].block1 != next->block1 || apc->layout[current_pos->position].block2 != next->block2)
	{
		...
		if ((current_pos != reference && current_pos->block1 != NOTHING_block) || (current_pos != reference && current_pos->block1 != NOTHING_block)) {
...
/**
 * Check a terminal or helipad, and if available, assign it.
 * @param v Aircraft looking for a free terminal or helipad.
 * @param i terminal to examine.
 * @return A terminal has been found, and has been assigned to the aircraft.
 */
static bool FreeTerminalSingle(Aircraft *v, byte i)
{
	Station *st = Station::Get(v->targetairport);
...
/**
 * Find a free Terminal checking Full Load orders.
 * @param v Aircraft
